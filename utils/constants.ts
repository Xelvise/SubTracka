...
// prettier-ignore
export const emailTemplates = dayIntervals.map(day => ({
    tag: "day " + day,
    generateSubject: (subName: string) => `${day === 1 ? "⚡ Final" : "📅"} Reminder: Your ${subName} Subscription Renews ${day === 1 ? "Tomorrow" : `in ${day} Days!`}`,
    generateBody: (data: EmailData) => generateEmailTemplate({ ...data, daysLeft: day }),
}))
...
// prettier-ignore
export const generateEmailTemplate = ({ username, subName, renewalDate, planName, price, paymentMethod, accountSettingsLink, supportLink, daysLeft }: EmailData) =>
